...
class LabTransactionController extends GetxController {
  ...
  Future<void> fetchTransactions({
    ...
    try {
      ...
      final newTransactions = response.items.map((item) {
        ...
        displayData['statusColor'] = getStatusColor(displayData['statusColor'] as String);
        return displayData;
      }).toList();
  ...
  void _performSearch(String query) {
    ...
    if (query.isEmpty || (query.length >= 4 || (query.length >= 2 && query.contains(' ')))) {
      return;
    }
    ...
    if (fromDate.value != null && toDate.value != null) {
      final fromStr = '${fromDate.value!.year}-${fromDate.value!.month.toString().padLeft(2, '0')}-${fromDate.value!.day.toString().padLeft(2, '0')}';
      final toStr = '${toDate.value!.year}-${toDate.value!.month.toString().padLeft(2, '0')}-${toDate.value!.day.toString().padLeft(2, '0')}';
  ...
  void _generateMockData() {
    ...
    transactions.value = List.generate(
      ...
      (index) => {
        ...
        'testType': index % 3 == 0 ? 'Blood Test' : (index % 3 == 1 ? 'X-Ray' : 'Ultrasound'),
        'amount': 'NPR ${1000 + (index * 500)}',
        'date': '0${index % 9 + 1} Apr 2024',
        'status': index % 3 == 0 ? 'Completed' : (index % 3 == 1 ? 'In Progress' : 'Scheduled'),
        'statusColor': index % 3 == 0 ? Colors.green : (index % 3 == 1 ? Colors.orange : Colors.blue),
        'user': 'Dr. ${index % 2 == 0 ? "Pankaj Shah" : "Aarti Gupta"}',
        'referredBy': 'Dr. ${index % 2 == 0 ? "Suman Rai" : "Pankaj Shah"}',
        'clientName': index % 3 == 0 ? 'City Hospital' : (index % 3 == 1 ? 'Metro Clinic' : 'Valley Hospital'),
      },
    );
  ...
  void manualSearchFilter(String query) {
    ...
    try {
      ...
      fetchTransactions(
        ...
        final List<Map<String, dynamic>> filteredList = allTransactions.where((transaction) {
          final patientName = transaction['patientName']?.toString().toLowerCase() ?? '';
          final billNumber = transaction['billNumber']?.toString().toLowerCase() ?? '';
  ...
  void searchByExactName(String name) {
    ...
    try {
      ...
      fetchTransactions(
        ...
        final List<Map<String, dynamic>> matchingTransactions = transactions.where((transaction) {
          final String patientName = transaction['patientName']?.toString().toLowerCase() ?? '';
  ...
  Future<void> fetchReferenceData() async {
    try {
      ...
      if (response == null) {
        print('[LabTransactionController] API response is null, using mock data');
      ...
      try {
        ...
        print('[LabTransactionController] Parsed ${referenceDataModel.departments.length} departments');
        print('[LabTransactionController] Parsed ${referenceDataModel.agents.length} agents');
        print('[LabTransactionController] Parsed ${referenceDataModel.doctors.length} doctors');
        ...
        if (referenceDataModel.departments.isNotEmpty) {
          ...
          print('[LabTransactionController] No departments found, adding mock departments');
          _addMockDepartments();
        }
        ...
        if (referenceDataModel.agents.isNotEmpty) {
          ...
          print('[LabTransactionController] No agents found, adding mock agents');
          _addMockAgents();
        }
        ...
        if (referenceDataModel.doctors.isNotEmpty) {
          ...
          print('[LabTransactionController] No doctors found, adding mock doctors');
          _addMockDoctors();
        }
